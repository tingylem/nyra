...
def belmont(url):
    ...
    for tr in sect.find_all('tr'):
       ...
       # link=tr.find('a',class_='text-style-body-2')
        ...
        if ml>5:
            if row[5]=='Dylan Davis' or row[5]=='Javier Castellano'or row[5]=='Irad Ortiz, Jr.'or row[5]=='John R. Velazquez':
                print(str(row[0]),str(row[2].partition('\n')[0]),str(row[5]),str(row[8]))
...
def main():
    ...
    baseurl="https://www.nyra.com//ajaxgetrace?tpl=FullEntryRaceTable&trackID=3&race="
